	...
	return {
		...
		appendWordInContext: function(msg) {
		 	...
		 	if(msg.word.length > 7) {
			 	...
			 	this.$el.find('.bigwords').prepend(e);
		 		...
		 		this.$el.find('.bigwords').children().each(function(i){
			 		var t = h*i+"px";
			 		$(this).css("top", t);
		 		});
		 		//console.log(this.$el.find('.bigwords > .bigText').length*this.lineHeight+"px");

		 		this.$el.find('.bigwords').css('height', this.$el.find('.bigwords > .bigText').length*this.lineHeight+"px");
		 	}		 	
		},
